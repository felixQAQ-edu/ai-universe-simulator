...
>
> 版本:v0.8(2026-09-06,ADR-004 刀 2 内容安全 · 主防线在生成之前:**单体骨架与融合骨架各加一段**
> 【绝境的写法 · 硬约束】(与 `prompts/event-loop.md` **逐字相同**——两份措辞一旦不同,world-gen 那份就成了
> 「为这一侧单独写的安全条款」,正是要防的孤立例外形状)。位置紧贴 `openingNarrative`:world-gen 不是「只产
> JSON」,它产的 JSON 里装着散文(`openingNarrative` 是 init 一次性下发、玩家最先读到的整段字),
> 这是最该管的地方而非勉强适用的地方。**运行时两份骨架都要改**(`SKELETON` + `FUSION_SKELETON`)——
> 本文件按【泄露硬约束】既有惯例只写一次,但 `FUSION_SKELETON` 是独立骨架,漏改则两个融合世界拿不到这段话。
> 一次修复路径(`REPAIR_HEAD`)不含骨架、本刀不动,已入 ADR-004 已知代价。文案定稿逐字不改、不加加粗。)
> 上一版:v0.7(2026-07-08,ADR-014 融合骨架参数化:FUSION_SKELETON 抽 per-combo 注入槽——【派生槽】致命轴
...
> **绝境的写法(ADR-004 刀 2)**:下段**逐字**与三处运行时骨架相同(**不加 markdown 加粗**,四面逐字节同串,
> `ContentSafetyPromptLockstepTest` 守护)。位置紧贴 `openingNarrative`——它是玩家最先读到的整段散文,
> 故这段是「怎么写散文字段」而非孤立安全条款。**本节写一次即覆盖融合**(同【泄露硬约束】的既有惯例:
> 融合段声明「输出格式骨架与上文单体完全相同」),但**运行时 `FUSION_SKELETON` 是独立一份骨架,必须同步改**——
> 漏改则两个融合世界的 world-gen 侧拿不到这段话,而 parity 只报变没变、不报该不该变。

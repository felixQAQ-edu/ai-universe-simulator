...
**白名单是许可不是义务**（Felix 2026-07-25 立字）：ADR-017 批这三个库是「**允许在需要时用**」，不是「要把三个都用上」。刀 2 的玉简签按压用 CSS `:active` + 时间感变量做，与 Motion 的 `whileTap` 等价——**为一个按压反馈引入整个库是纯债**。**挂账**：若刀 3 / 刀 4 走完 Motion 仍一次未用，刀 4 收口时评估**收窄白名单**（白名单里躺着从没用过的库，本身就是可还的债）。

...
⚠️ **一条指针,不改被指的那句正文**:上面那条挂账 3 第 3 条把「延迟三项」一并判为
「链路上**从未有过**时间锚」——**该理由对其中的「回合耗时」自 2026-09-15 起不再成立**
(锚点已打)。⚠️ 它**不是那条挂账的 pre-existing 事实错,是本刀造成的状态变化**,
故按「不顺手改」**只在此放指针、留待裁定**;而「延迟三项移出**统计脚本**范围」那半
**照旧成立**(第 5 条维持冻结)。

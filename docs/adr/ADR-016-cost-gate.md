...
### 冒烟前置检查(部署待验分支必做,2026-07-22 弯路教训立字)

首轮线上冒烟「设 `INIT=2` 后连开多局均不被拦」,一度误判为 header 注入 bug——**实为部署陈旧**:同源单容器(ADR-015)一个镜像同时打包 web dist + server jar,`fly deploy` 未真正换镜像时,线上跑的是旧产物(既无后端 quota 代码、前端也不带 `X-Device-Id`),`INIT=2` secret 只是没人读的环境变量。**根因由 bundle 哈希对比坐实**:线上 `index-CD_GKrfu.js`(X-Device-Id 出现 0 次)≠ 本地现构建 `index-CIRy5prQ.js`(2 次)。

立字:**部署待验分支,先确认镜像真换了、再验功能**——功能级冒烟前跑一条零成本检查:

```bash
# 线上首页引用的 bundle 名应等于本地 `npm run build` 产出的哈希;不等 = 镜像没换,先重部署
curl -s https://wanjie-ai.fly.dev/ | grep -o 'assets/index-[^"]*\.js'
# 该 bundle 应含本批前端接线证据(X-Device-Id 两处 = init + turn)
curl -s "https://wanjie-ai.fly.dev/$(curl -s https://wanjie-ai.fly.dev/ | grep -o 'assets/index-[^\"]*\.js' | head -1)" | grep -c X-Device-Id
```

单元测试抓不到「部署了旧镜像」(它只锁 api 层注入不回归);抓到它的是上面这招抓线上实物。教训沉淀同步入 [Fly 部署 runbook](../phase3-fly-deploy-runbook.md) §冒烟。

## 实际效果(事后补充)

**线上冒烟全过(2026-07-22,Fly.io syd,临时压低阈值 `INIT=2`/`TURNS=3`,真 key)**:

| 验收项 | 结果 |
|--------|------|
| 软闸 init(单键日局数) | 第 3 局拦「今日新世界名额已满」;Network 实证 init 请求头带 `x-device-id` ✅ |
| 软闸 turn(单键日回合数) | 第 4 回合拦「今日回合名额已满」,决策圈保留、会话停 AWAITING 可续(守卫 0 相位零触碰实证)✅ |
| 真闸月顶(¥) | `MONTHLY=0.001` 进来即拦(月累计已达 ¥0.0132 > 阈值);**redeploy 后仍拦**(月累计跨重启守住)✅ |
| 月落盘(原子写 + /data 卷) | `/data/quota-2026-07.json = {"month":"2026-07","spentCny":0.013177…}` ✅ |
| 前端文案 | 首轮 init 拦截屏标题误用「世界生成失败」(复用 world-gen 分支),已修为 `errorCode` 区分「今日名额已满」vs 真失败(`00c3254`)✅ |

**记账精度佐证**:月落盘 `spentCny=0.013177…`(数局真 key 累计)与 ADR-001 price 三段(hit ¥0.02 / miss ¥1.0 / output ¥2.0,CNY/1M)× 线上真实 usage 逐笔相符,量级正确——「跟官方价走、按 usage 块累加」的估算口径成立。

*软启动放量后再回填:(a) 四路计数与 ¥ 账本的日/月曲线;(b) 是否出现真人撞软闸(阈值哲学的实证);(c) 月累计与 DeepSeek 官方账单的偏差量级;(d) 429/quota_exceeded 的触发频次与来源分布(脚本刷是否真的来了)。*

...
- **实现进度**(活状态格):**刀 1 已合并(2026-09-26);刀 2 待校勘 / 未合并(分支 `claude/adr025-knife2-history-api`);刀 3–4 未起**
  (快照列用 PG `json` 不用 JSONB,理由见 V1 迁移注释)。默认 profile 行为不变;线上切库属刀 4。
- **决策者**:Felix
...
10. **DB 故障在 `/actuator/health` 上不可见(刀 1 实现时补记)**:`pg` profile 下关掉了 db 健康指示器
    (`management.health.db.enabled: false`)—— 理由与决策 3 同源:DB 抖动不该让平台健康检查把一台
    还能玩的机器判死重启(内存仍是权威,persist 失败只丢落库)。**代价**:DB 挂了,`/actuator/health` 仍报 UP;
    唯一的现形处是 persist 的 ERROR 日志(`[session-store:pg] ... 落库失败`)。它与工程债
    「上游硬失败今天不可见」是同一族(外部依赖失败没有任何一层在看),且同样依赖「日志是瞬时的」那条挂账。
...
2. **开场叙事 —— 2026-09-26 Felix 定**:**进历史,作 turn 0**,在刀 1 内、于 create 那个 persist 点写入;
   ...
   - **载体**:`GameSession` 带上它(一个只在 create 那次 persist 被读的值);
     > 订正(2026-09-26,刀 1 实现时):「只在 create 那次 persist 被读」**与实现不符**。实现是:该值随会话
     > 常驻,**每次** persist 都把它作 turn 0 放进候选,再按「turn 大于库内已存最大 turn」过滤 —— 故
     > create 那次 persist 若失败,**下一次成功 persist 会补上 turn 0**(库里还一条事件都没有时)。
     > 补不上的只有一种情形:create 失败之后、下一次成功 persist 之前**进程重启**(回载会话该值恒为空)。
     > 原句保留不覆写,口径以本订正为准。
...
- **刀 2 · 只读 API**:历史读接口 + 消毒硬闸 + 两种缺口的区分 + 文件 profile 下返回「本环境无历史」(已决 4)。只读、无副作用,
  可单独回滚。
  > 加注(2026-09-26,刀 2 实现时):**分页按回合号区间**(`?afterTurn=N`,每页固定 100 个回合号,不按条数、不用 offset);
  > 前提是**一局回合数无硬上界** —— `LifeStageTables` 与 `TurnPromptBuilder` 注释明写「不写死回合数上限」,
  > ADR-020 刀 4 实测一局 200 回合未收束。决策正文不变。
...
- **刀 4 · 线上切换与真机冒烟**:`fly` 侧的 DB 创建、secret、profile 切换**全部 Felix 亲手**
  ...
  **刀 4 待办(只记,刀 1 未测)**:`pg` 下 Flyway 在启动时跑迁移;若启动时 DB 不可达,推测应用会
  **起不来**,而不是走 `loadAll` 那条「无存档启动」分支 —— 此推测**未经测试**。刀 4 须核实并定启动行为
  (与 Fly 健康检查 / 重启策略一起)。

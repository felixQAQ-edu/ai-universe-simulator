...
| **返回键缺失**(刀 4 真机暴露,2026-07-29) | 玩家进局后**没有任何导航出口**,只能靠浏览器返回键退出。**不属四刀任一范围** —— 四刀做的都是皮肤(氛围/数值/选项/正文形态),而这是**导航层缺失**:它一直存在,只是四刀把四个世界都做得值得反复进出之后才暴露出来。**不做,记着**;**下一步由 Felix 统一提方案**(涉及「退出到选择屏是否弃局 / 与续局 saveId 的关系 / 是否需要二次确认」,是产品决策不是视觉决策) |

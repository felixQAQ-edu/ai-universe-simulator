...
### 3.1.4 部署前:确认本地工作副本不落后(每次 `fly deploy` **前**必做)

> **弯路教训立字(2026-09-17,层 1 第 4 条锚点首次真机采样)**:要部署的是 `7e0eef7`,
> 而实际打出去的是 **`e6d6ee2`** —— 因为 **CC 在自己的容器里合并并 push,而 Felix 的 Mac
> 工作副本从没拉过**;`fly deploy` 用的是 **Felix 的**工作副本。
> 构建成功、健康检查通过、`GIT_SHA` 也正常打上 —— **全绿,但打的是旧那份。**
>
> ⚠️ **而 §3.1.5 当时也是绿的,这才是要记的那一半**:它比的是「线上 SHA vs **本地** HEAD」,
> 而本地 HEAD 本身就是旧的(Mac `e6d6ee2` == 线上 `e6d6ee2`,**相等 → 放行**)。
> **它回答的是「镜像换成了这台机器上的那份吗」,不是「换成了 `main` 那份吗」。**
> ⚠️ 故这不是「漏了一步」,是 **[ADR-018 §4.13](adr/ADR-018-base-world-visual-migration-and-severity-contract.md) 一族**:
> **守护为真,而被守的事照旧发生了** —— **一个当时全绿的检查,是这次事故的一部分。**
> 发现它靠的是「我知道期望值应该是 `7e0eef7`」这条**本地 HEAD 之外**的信息。

```sh
git fetch origin
git status -sb          # 期望:## main...origin/main(无 [behind N])
```

**判定**:

- 无 `behind` → 本地就是 `origin/main`,继续 §3.1.5,那里的「本地期望值」这才可信。
- 有 `[behind N]` → **先 `git pull --ff-only` 再 deploy**;不拉就部署 = 把旧代码推上线,
  且 §3.1.5 **不会报错**。

> **口径立字**:**「CC 报了合并成功」≠「你的工作副本有那份代码」。**
> CC 的合并与 push 发生在它自己的容器里,那台机器不是部署源;
> 这是本文头部那条**分工红线**
> (`fly` 命令 Felix 亲手)的直接结果 —— **部署源与代码源是两台机器,它们不会自己同步。**
...
> ⚠️ **这个「本地期望值」只有在 §3.1.4 通过之后才可信** —— 本地 HEAD 落后时,
> 它与线上会**相等而放行**(2026-09-17 实发,见 §3.1.4)。本节口径不变,只是补上它的前提。

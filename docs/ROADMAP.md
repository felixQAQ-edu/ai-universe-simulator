...
| 当前阶段 | Phase 1 · 单模式 H5 闭环(规则怪谈)进行中 🟨(后端骨架已搭、真实 DeepSeek 已接入 SSE 通路、event-loop 数据面内核移植落地) |
...
- **完成(event-loop 内核移植 · 数据面)**:按内核先行切分起 Phase 1 event-loop 第一批,落 `server/.../engine/`。移植 bake-off 已验证逻辑为 Java(parity 锁死):`Engine.apply()`(§5 序列 1–10:绝对值/跳变>40 标记不拒绝/clamp/triggered+discovered/log 折叠 LOG_KEEP=4 抽取式/结局判定/§5 兜底结局 id 补丁)、`GameSchemas`(WORLD/TURN_SCHEMA + §10 结局回合 actions 放宽)、`LeakDetector`(字段名 + hiddenLogic 逐字≥8 子串,§1c 遥测语义)、`LooseJson`(剥围栏/取首 {…}/降级)。新建 `Engine.toClientState()` 消毒投影(递归剥 isTrue/hiddenLogic/isCorrect/groundTruth,出网必经)。**golden parity**:`bakeoff/replay_golden.py` 重放 30 event-loop + 8 world-gen 录制 raw 经 Python `Engine` 导出端状态夹具,Java 移植逐字段 == Python 三路径(B1/B2/B3 × T1–T10);三路径均干净跑满 10 回合(ongoing/未触底)→ §5 兜底/结局结算/跳变标记不被 golden 覆盖,各配专门单测。`mvn test` 50 绿(35 新 + 15 旧),commit `d8cf95f` @ `phase1/event-loop-core`。**0b 改判**:Boot 4.1 = Jackson 3,networknt(Jackson 2)会造双 Jackson 世界 + 顶撞单一 ObjectNode 决策 → 手写校验器,零新依赖(见 F-007)。**纯数据面,未起状态机/SSE/EventLoopService/守卫/降级/prompts。**
- **下周计划**:按 [event-loop 规格](phase1-event-loop-contract-and-state-machine.md)起 event-loop 实现(`EventLoopService` 哨兵切分 + 叙事回灌 + 消毒投影 + 忙态/合法性/兜底/降级,各配单测;改造 `prompts/event-loop.md`:prose+哨兵+尾巴去 narrative);手动集成冒烟(`DEEPSEEK_API_KEY` 就位后 curl 看真实逐字流);Phase 1 排期纳入 ICP 备案。

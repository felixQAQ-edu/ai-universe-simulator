...
| 当前阶段 | Phase 2 · 多模式 **进行中 🟨**(第一批=末日生存 `apocalypse` 落地、真 key 冒烟**验通**(2026-06-24):world-gen 首过 3/3 零修复 + AI 落 hunger 衰减稳定 40→0/每回合 -5 + 整局闭环贯通 + 叙事无泄露,多模式管线可扩展性验证通过)。ADR-008(C 偏 A 数值无知 + per-archetype 元数据)采纳,核心一次性 key-agnostic 泛化经 golden parity 守不回归。下一步(选择 UI 小批 / 第二个模式 / 衰减率体感微调)在 Project 窗口讨论中定;Phase 1 单模式闭环里程碑已于 2026-06-23 达成;未碰 ICP 备案 / CloudBase 部署,挂后续 |
...
| Phase 2 | W10–16 | 多模式 + 分享 + 云存档 + 成本控制 | 🟨 | | [ADR-008](adr/ADR-008-multi-mode-extension-architecture.md) · [第一批设计稿](phase2-batch1-apocalypse-mode.md) · `server/.../archetype/` · CONTEXT v0.7 | **第一批·末日生存(`apocalypse`)已落地 + 真 key 冒烟验通(2026-06-24)**:验多模式管线能跑通=给后续所有模式立模板。**ADR-008 采纳**(C 偏 A:引擎/校验对数值 key 语义无知 + per-archetype 轻量元数据非强制校验;衰减 AI 落引擎无知;world-gen 通用骨架+注入块;init archetype 入参;逐模式独立 world-gen 冒烟硬验收门)。**事实订正**:核心实为 key-fixed 到 `{hp,san}`(非 brief 假设的已 key-agnostic),据 Felix 拍板**一次性机械泛化为 key-agnostic**(Engine/校验器/buildDelta 遍历 attributes map,只做「字段名→字典遍历」、结算/clamp/跳变/兜底逻辑不动),**golden parity 139 逐字节守 `{hp,san}` 零回归**。6 commit:ADR 落档(`b4b598d`)+ 核心泛化(`ee0435d`)+ 元数据(`b35d023`)+ 提示词通用骨架+注入(`7a126d3`)+ init 入参+校验+轴元数据下发(`5be122c`)+ 前端面板按元数据动态渲染(`f53c14a`)。`mvn test` **154 绿**(139 baseline 字节不变 + 4 key-agnostic + 5 元数据 + 2 提示词注入 + 4 init)、前端 `npm test` **18 绿** + lint/build 全绿。**真 key 冒烟验通(Felix 读数拍板)**:world-gen 首次有效率 3/3 零修复、格式完整 hunger 轴齐、AI 落 hunger 衰减稳定(40→35→…→0 每回合 -5、只动衰减轴、推到 turn 15 未忘、触底 `starved_to_death` 端到端)、叙事无泄露、整局闭环真 key 首次贯通、生存压力体感真实 → ADR-008 决策 1(key-agnostic 泛化)+ 决策 2(AI 落衰减)实测成立。ff 合并 `main`。**待办(独立小活,非本批)**:`/tmp` 末日真实 raw + SSE 转录接进测试夹具替换合成数据(见 FINDINGS)。**下一步在 Project 窗口讨论**(选择 UI 小批 / 第二个模式 / 衰减率体感微调),定了再起;分享 / 云存档 / 成本控制仍未起。 |
...
- **下周计划**:**不起下一批**——下一步(前端 archetype 选择 UI 小批 / 第二个模式 `cultivation`/`cyberpunk` 各自独立批 + 独立 world-gen 冒烟 / 衰减率体感微调)仍在 Project 窗口讨论,定了再单开 Code 窗口。并行待办照旧:视觉打磨(`frontend-design` skill)、ICP 备案 + CloudBase 部署接缝、Phase 2 后续主线(分享长图 / 账号云存档 / 成本控制)。

...
| 当前阶段 | Phase 2 · 多模式 **进行中 🟨**(第一批=末日生存 `apocalypse` 落地、真 key 冒烟**验通**(2026-06-24);A 计划=archetype 选择 UI + 每世界专属卡片氛围**落地 e2e 验通**(2026-06-25):玩家可选模式进游戏、`GET /api/archetypes` 目录、卡片即世界预告片(纯 CSS,规则怪谈冷蓝诡异闪烁 / 末日锈土沙尘))。ADR-008(C 偏 A 数值无知 + per-archetype 元数据)采纳,核心一次性 key-agnostic 泛化经 golden parity 守不回归。下一步=**加世界流水线起步,下一个=克苏鲁**(规则怪谈近亲、最便宜,练手再证流水线,见[世界库 backlog](world-library-expansion-backlog.md));设计在 Project 窗口起、实现新 Code 窗口起。Phase 1 单模式闭环里程碑已于 2026-06-23 达成;未碰 ICP 备案 / CloudBase 部署,挂后续 |
...
- **下周计划**:进**加世界流水线起步,下一个=克苏鲁**(`cthulhu`/规则怪谈近亲、最便宜,练手再证「加世界=一条元数据 + 一个 world-gen 注入块 + 一次独立真 key world-gen 冒烟」的流水线;world-gen 最高风险生成步,规则怪谈/末日成功不外推)——**克苏鲁是独立工作单元**:设计在 Project 窗口起、实现新 Code 窗口起(本窗不起)。并行待办照旧:视觉打磨(`frontend-design` skill)、ICP 备案 + CloudBase 部署接缝、Phase 2 后续主线(分享长图 / 账号云存档 / 成本控制)。

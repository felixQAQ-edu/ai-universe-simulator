...
- **INITIALIZING**:一发 **world-gen 胖调用**,产出 `world + character(初值) + rules + endings + 初始 availableActions`,`validate_world` 校验。
  ...
  > Phase 1 单模式**不拆** char/rule/ending-gen(省 round trip、已验证形态);分步留给将来 fusion。~~开场叙事可流式(复用 §4)。~~ → **覆盖:开场叙事 reveal 不流式,作 `openingNarrative` 字段一次性下发,理由见 [ADR-007](adr/ADR-007-world-gen-wire-protocol.md) / [world-gen 设计稿](phase1-world-gen-and-initialization.md)**(冒烟数据让我们反过来选可靠性优先:world-gen 保 json_object,不为开场 vibe 丢它)。
...
- **AWAITING_ACTION**:已下发 `availableActions`,等玩家选 id。
  **忙态守卫(🆕,已实现)**:仅此态接受动作;`GENERATING`/`SETTLING`/`ENDED` 期间到来的动作一律拒(防一回合被双花)。
  并发原语:每个 saveId 的相位活在内存 `ConcurrentHashMap<saveId, TurnPhase>`(`GameSessionManager` 承载),
  入 `GENERATING` 走 `AtomicReference.compareAndSet(AWAITING_ACTION, GENERATING)`(**非读后写**)——两线程同回合并发恰一个过。
...
- **不变量(🆕,本批确认):回灌后才校验,绝不校验裸 tail**。裸尾巴无 `narrative` 字段,校验它必误挂在
  `narrative` minLength 上。`reinfuseAndValidate` / 修复 probe 一律先 `TurnReinfuser.reinfuse`(灌回 canonical 叙事 N)再 `validateTurn`;
  修复发也回灌**同一个 N**(不让修复改写已流出叙事)。
...
- ~~`TURN_SCHEMA` 在结局回合仍 require `availableActions`(minItems 2)~~ → **已落地(第一批)**:`GameSchemas.validateTurn`
  在 `ending.reached == true` 时放宽 `availableActions` 为可空(minItems 0),客户端在 `ending.reached` 时忽略 actions。

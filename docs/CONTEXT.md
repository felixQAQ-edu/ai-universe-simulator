...
17. **持久化边界(ADR-015,Slice 1–3 落地 + Fly.io syd 线上真机验通后回写,2026-07-20)**:最小续局落盘(每 saveId 一个 JSON,不引数据库)的五条约定——**(1) 落盘 = 视图 1 全量投影**(含 `isTrue`/`hiddenLogic`),与视图 2(喂模型)/ 视图 3(客户端消毒)三分**不得混用**(与 §三.9 同源);**(2) 落盘格式** = 引擎七字段 `{schemaVersion, world, state{turn/status/timeline/log/logSummary}, triggered, issues}` + session 层 `currentActions`/`phaseHint`,`Engine.restore` 构造前全量校验、非法输入**拒载不半载**;**(3) 轴语义集不落盘**——由 `world.archetypes` 经 `ArchetypeRegistry.resolveAxes` 原路重派生(播种/回载/续局三处单一真理源);**(4) 落盘目录必须在 web 根之外**(启动断言,违者拒启)且 `persist` best-effort 绝不抛(写盘是增值服务非关键路径,磁盘故障丢续局不杀活回合);**(5) 崩溃回滚一回合是特性非 bug**(写盘在回合完整结算后,盘上永远是最后一个完整回合)。落盘引用 `schemaVersion` 现值(仍 "0.4")、非 wire schema 字段变更;实现与验收细节(restore 守护测试 / 部署冒烟)见 ADR-015 附录 A/B。平台细节(Fly/fly.toml/区域)属部署实现非约定,不进本节。

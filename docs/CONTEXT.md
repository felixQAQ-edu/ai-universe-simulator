...
16. **混合模式融合协议(ADR-012 轴合并 + ADR-013 融合协议,round 1 端到端验通 2026-07-08;ADR-014 骨架参数化 + 第二组合,round 1.5 端到端验通 2026-07-14)**:混合模式(`mode:"hybrid"`、`archetypes` 2–3)把多套设定调和成**一个自洽世界**(非轮流播/拼接)。round 1 彩蛋=**修仙 × 规则怪谈「识海遗蜕」**(host=修仙);round 1.5=**规则怪谈 × 末日「守则即补给」缺页的人防工程**(host=rules_creepy,hunger 换皮「补给」、首例三致命轴 {hp,san,hunger})。**引擎/校验/`schemaVersion`(保 "0.4")一律不动**——融合只在**播种层 + 提示词 + 前端**三处;引擎侧融合世界只是「一个有 N 轴的普通世界」,照旧遍历 attributes(守 §三.5 引擎数值无知)。融合提示词=**参数化骨架 + per-combo 槽位**(ADR-014:派生槽=致命轴清单/结局条数从 `fusedAxes` 算,文案槽=真假称呼/资源经济/裁决口吻/附加指令;安全规矩全在骨架单点维护)。八条约定:
    ...
    **(7) 时间尺度是 per-combo 隐性设计参数(ADR-014 round 1.5,FINDINGS F-017)**:每个融合组合的**时间结构必须在 combo 槽位里明确**——识海遗蜕=单夜封闭(自洽,恢复手段随时可执行);缺页的人防工程=**多日昼夜循环**(白天=补给窗口、夜晚=守则狩猎时间)。不明确时间尺度,组合的资源经济可能被叙事时间结构架空(人防三局断粮饿毙的根因之一:整局被压缩在一夜、日间补给通道无处存在)。逐回合消耗类 behaviorHint 应**挂钩行为**(奔逃/劳作快耗、静卧/休整轻耗)而非按回合无条件扣,让「熬过时间」成为可存活策略。

    **(8) 恢复手段须可自主执行、或兑现窗口明确(ADR-014 round 1.5,FINDINGS F-017)**:约定 (6) 的「资源经济内生」再收紧一档——恢复手段要么**可自主执行**(识海的参悟/调息/丹药),要么被时间/他人门控(人防的配给日/搜刮/换粮)但**配套明确的时段语义**(窗口何时开、关着时给过渡行动)。发牌类指令一律写成**兑现语义**:玩家选了恢复选项,`stateUpdate` 须给与叙事相称的实际增量,「叙事说领到、数值不动」是错误;时段无法兑现则选项不得以「领取/获取」面目出现(不许发空头牌)。
...
17. **持久化边界(ADR-015,Slice 1–3 落地 + Fly.io syd 线上真机验通后回写,2026-07-20)**:最小续局落盘(每 saveId 一个 JSON,不引数据库)的五条约定——**(1) 落盘 = 视图 1 全量投影**(含 `isTrue`/`hiddenLogic`),与视图 2(喂模型)/ 视图 3(客户端消毒)三分**不得混用**(与 §三.9 同源);**(2) 落盘格式** = 引擎七字段 `{schemaVersion, world, state{turn/status/timeline/log/logSummary}, triggered, issues}` + session 层 `currentActions`/`phaseHint`,`Engine.restore` 构造前全量校验、非法输入**拒载不半载**;**(3) 轴语义集不落盘**——由 `world.archetypes` 经 `ArchetypeRegistry.resolveAxes` 原路重派生(播种/回载/续局三处单一真理源);**(4) 落盘目录必须在 web 根之外**(启动断言,违者拒启)且 `persist` best-effort 绝不抛(写盘是增值服务非关键路径,磁盘故障丢续局不杀活回合);**(5) 崩溃回滚一回合是特性非 bug**(写盘在回合完整结算后,盘上永远是最后一个完整回合)。
    ⚠️ **触发器补充(ADR-023,2026-09-12):判语一字未动,补的是「什么会触发它」** —— 不止「崩溃」。
    **正常使用中的断流**(玩家点返回 / 切后台 / 网络断)同样会造成这个暴露:SSE 写抛出 →
    `TurnStateMachine` 的 catch 接住 → **`persist` 在那之前一行、根本没跑到**,于是内存 N+1 而盘上仍是 N。
    **与崩溃的差别只有一个,但很要紧:玩家无感** —— 他手里那一局比服务端旧一个回合,而他不知道,
...
| **v1.6** | 2026-09-12 | **§三.17 第五条补一条触发器(ADR-023),判语一字未动**:原句「崩溃回滚一回合是特性非 bug(写盘在回合完整结算后,盘上永远是最后一个完整回合)」**没有说假话** —— 盘上确实是最后一个完整回合,此刻崩溃确实回滚一回合;**它缺的是触发器不是判语**,故本次**补不覆写**(覆写一句没说错的话是另一种漂移)。补的内容:**不止「崩溃」** —— **正常使用中的断流**(玩家点返回 / 切后台 / 网络断)会让 SSE 写抛出、`persist` 被跳过,造成同样的暴露,**且与崩溃有一个要紧的差别:玩家无感**(他手里那一局比服务端旧一个回合而不自知,再点一次会让服务端推进第二个回合 —— 旧选项 id 跨回合稳定为 A/B/C/D,合法性守卫拦不住)。该重复推进已由 ADR-023 的游标比对拒掉(409 `turn_stale` + 前端拉 `/state` 同步);⚠️ **「内存 N+1 / 盘 N」这一偏差本身仍未治**,解冻绑数据库那一刀。**JSON `schemaVersion` 不动(仍 "0.4")**(无 wire schema 字段变更,落盘格式一字未改);仅 CONTEXT 文档版本升 v1.6。 |
| **v1.5** | 2026-07-20 | **持久化边界约定回写(ADR-015 三刀全落地 + Fly.io syd 线上真机验通后立字)**:新增 **§三.17 持久化边界**五条(视图 1 全量与三视图不混用 / 落盘七字段 + session 两字段、拒载不半载 / 轴语义集不落盘经 registry 原路重派生 / 落盘目录 web 根之外 + persist best-effort / 崩溃回滚一回合=特性)。按 ADR-015 暂缓条款(循 ADR-013 先例「待验通再立字」)在附录 A restore 守护测试首过 + 线上落盘进卷/redeploy 回载续局真机检验通过后回写,非空头约定。**JSON `schemaVersion` 不动(仍 "0.4")**(落盘引用现值,非 wire schema 字段变更);平台细节(Fly/fly.toml)不进约定层,留 ADR-015/runbook。仅 CONTEXT 文档版本升 v1.5。 |
| **v1.4** | 2026-07-14 | **round 1.5「守则即补给」(ADR-014)端到端验通,§三.16 补两条融合协议约定**:§三.16 头部更新(ADR-014 骨架参数化 + 第二组合=缺页的人防工程,融合提示词=参数化骨架 + per-combo 槽位,六条约定扩为八条);新增 **(7) 时间尺度是 per-combo 隐性设计参数**(识海=单夜封闭 / 人防=多日昼夜循环,combo 槽位须明确;逐回合消耗类 behaviorHint 挂钩行为而非无条件扣)+ **(8) 恢复手段须可自主执行、或兑现窗口明确**(被时间/他人门控的通道配套时段语义;发牌类指令一律写成兑现语义,不许发空头牌)——两条均出自 FINDINGS **F-017**(人防工程经济死局,三局断粮饿毙 → E'/E'' 双刀根治,第 4 局终验通过:死于守则机制而非经济死局)。引擎/校验/`schemaVersion`(仍 "0.4")零动;server 246 / web 62 全绿、golden 与识海 prompt 零回归。仅 CONTEXT 文档版本升 v1.4。 |

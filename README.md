...
- [ADR-022](docs/adr/ADR-022-turn-admission-and-rejection-semantics.md) — 回合线程池准入与拒绝语义:给回合路径装一道**并发准入**,让「此刻太挤」成为一个**有名字、有文案、有日志、玩家看得懂**的拒绝(治[工程债 §1.1](docs/backlog-engineering-debt.md):`newCachedThreadPool` 无上限 × SSE 阻塞长连接 = 模型变慢与恶意请求双重放大,512MB 单机通向 `OOM: unable to create native thread`;⚠️ ADR-016 的软闸是**日次数**闸不是并发闸,一个脚本一秒打满当天额度**账面完全合规而线程池已经炸了**)。**核心立字 = 准入必须在领线程之前发生** —— 换池不解决问题(上限设成 N,一批必然被拒的请求照样各占一个名额跑到被拒),**故这不是换一行 `Executors.*`**;**「此刻太挤」不归 ADR-016「排队不做」管辖**(那条立字主语是**闸门触发**、理由挂在「明天再来」上、管**日级**状态,而「太挤」是**秒级**的:下一次点击就可能成功)——**ADR-016 一字不动,且本刀不触发它的重新审视条件**(不排队、也不是等额度;这句写进 ADR 是为了防后来者读成解冻);**不排队 = 队列容量为零**;**准入不是判断是占位**(配额在名额之内跑);**池准入在前、配额在后**(被池拒绝的玩家什么都没得到,扣额度是双重惩罚);**守卫 1 前移到 controller、守卫 0/2 留原地**(守卫 2 前移要引入一个**必须永不失败的写操作** —— CAS 成功后若被拒,相位停在 `GENERATING` 无人放回 = **该存档永久 busy,只有进程重启才解开**);**显式接受 ADR-016 守卫顺序反转**(非法动作不再消耗配额,**不许是搬家的静默副产品**);**拒绝走 HTTP 不走 SSE**(SSE 路线**必须先领一个线程才能说这句话**,而线程正是要省的东西)。**方案取舍的决定性一条**:有界池 + `AbortPolicy` 归还不会漏,**但它的在途数只能靠手工维护一个计数器**(`getActiveCount()` javadoc **自陈 approximate**,而自陈近似的读数不能拿来签字说机制生效)—— 信号量方案**让「在途数」这个最容易撒谎的读数失去了撒谎的能力**(它是准入决定本身的投影,**没有可漏掉的更新路径,因为它根本没有更新路径**);⚠️ **不是「C 在补 A 的短」**:两者各有一处会漏,选 C 是因为**它漏的那处能被结构消灭,而 A 漏的那处写在 JDK javadoc 里、封装不掉**。**归还在结构上不可遗忘**(准入对象同时拥有获取与提交、只暴露 `submit(saveId, work)`,**调用方拿不到那个名额** —— 取消那个步骤本身,不是更小心地执行它),**归还责任的交接点画死**并各有一条测试。**`@PreDestroy` 被迫改**(原口径「若被迫改则停下报告」已履行):池归准入对象、自带 `@PreDestroy`;**否决「只传引用故一字不动」那种形态 —— 它字面上做得到,而那正是不选它的理由**(字段含义已搬走而那行看起来毫无变化,正是 `GameController:46` 那句 `StreamController` 孤儿注释形成的前一步:**立字失去宿主不是一次断裂,是一次「看起来没变」**)。七项裁定:**N=8**(承重的是「8MB 栈堆外可忽略」与「满负荷约 55 分钟先撞 ADR-016 日闸」;⚠️「真实并发 ≈1」**降级为量级旁证不作支点**,累计量答不了并发问题;⚠️ **「只管不死」限定到 turn 路径**,init 仍阻塞占 Tomcat 线程且上限 200 无人管,**不限定就是超卖**)/ **503 且不带 `Retry-After`**(带上等于发一张自动重试许可证,而**齐步重试正是过载时最坏的形态**)/ **`server_at_capacity`**(⚠️ 不得复用 `busy` —— 那是 per-save 的「上一回合仍在结算」,共用会让玩家**一半时间收到错的解释**)/ 文案给**秒级**预期 + notice 黄条 + 决策圈保留可点 + 相位零触碰 / 在途数与准入原语同一真相源 / 三刀切分 / **不新建接缝接口**(`QuotaGate` 有接口是因为它有真实的「关掉」形态与多消费方,准入两条都不成立);**关闭语义原样承继**(`@PreDestroy` body 仍是 `pool.shutdown()` 一行 —— **不 `awaitTermination`、不做「停止受理」**,在途回合由进程退出硬掐,同 ADR-015 已知代价 1 一族,**仍挂账**;⚠️ 由此立禁令:**不许把 `RejectedExecutionException` 映射成 `server_at_capacity`** —— 「此刻太挤,过几秒再来」与「服务器正在关,过几秒也没用」是两种情形,共用一个码比 `busy`/`server_busy` 那种「会被看错」更糟:**它连看错的机会都不给**);三刀顺序不可换(1 前端语义映射、`sse.ts` 回到它自陈的边界·**可独立上线**·顺带清 `409→busy` 死映射 / 2 后端准入 + 守卫 1 前移 + 观测面 / 3 真机冒烟,⚠️ **必须用两个存档** —— 单存档第二次点击先撞忙态守卫拿到 `busy`,**那是 ADR-018 §4.18「观测指标要与被测对象匹配」的原样重演**);引擎 / 校验 / golden / prompt lockstep / `schemaVersion`(保 0.4)一律零动

...
- **内容安全实现未收口**:[ADR-004](docs/adr/ADR-004-content-safety.md) 已落档(2026-09-06),
  四刀切分**已走完刀 1 与刀 2**(2026-09-07)——`moderation` 包已撤(**它撤掉的是一个恒等函数,
  不是一道关** —— 解冻条件见 ADR-004 §最终决策三)、自伤硬禁已写进 prompt 骨架;
  **刀 3(扫描器)与刀 4(真机冒烟)未起,且这两刀之前它不算收口**;
...
- **内容安全**:**不接外部审核 API** —— 主防线是 prompt 侧硬禁,检测器只做事后遥测(ADR-004 已落档,**实现走完刀 1/刀 2,刀 3/刀 4 未起**)
...
- [ADR-025](docs/adr/ADR-025-narrative-history.md) — 叙事历史:让「回看这一局」成为一个能力,PostgreSQL 只是它的存储 ⤷ *提议,未采纳*

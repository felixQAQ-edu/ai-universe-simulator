...
public final class TurnPromptBuilder {
	 ...
	 *
	 * <p><b>⚠️《动物人生》第 (8) 条断裂素材为什么落这一侧(F-028 修法,ADR-021 §断裂素材补记)</b>:
	 * 断裂是<b>一次性事件</b>,按「读几次」的字面读法像是 world-gen 侧的东西 —— 但那条判据问的是
	 * <b>「在哪一次调用里生效」</b>,不是「事件发生几次」。world-gen 那一次调用产的是世界的初始 JSON,
	 * <b>它产不出第 15 回合的正文</b>;放在那一侧唯一的送达途径是让它写进 {@code background},
	 * 而那既不可靠、又把断裂提前剧透给玩家。同 <b>F-025</b>:软约束要挂在模型<b>那一回合</b>已经拿得到的东西上。
	 *
	 * <p><b>⚠️ 给物,不给顺序</b>:清单只规定「这三个回合里必须出现这些东西」,<b>不规定哪一回合写哪一样</b>——
	 * 写死分配等于给模型排剧本,每一局都会长得一样,重玩价值归零。
	 * 这是<b>「必须发生」与「必须这样发生」的分界</b>。
	 */

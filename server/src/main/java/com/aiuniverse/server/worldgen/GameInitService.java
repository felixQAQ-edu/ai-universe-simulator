 ...
 * → 解析初始动作(FALLBACK 兜底)→ 播种会话 → 建消毒 init 响应。
 *
...
public class GameInitService {
	...
	public GameInitService(WorldGenService worldGen, GameSessionManager sessions,
			ArchetypeRegistry archetypes, ObjectMapper mapper) {
	...
	public InitResponse init(List<String> archetypeIds) {
		...
		// 1. 提取 transient openingNarrative,再从 world 根剥除(不入持久化 state)。
		String opening = world.path("openingNarrative").asString("");
		...
		// 3. 播种会话(INITIALIZING→PLAYING,turn FSM AWAITING_ACTION)。三份元数据派生信息据解析出的轴集算
